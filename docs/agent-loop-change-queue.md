...
| 2 | haidao-agent-loop-trigger-api | 暴露 `/api/weibo/agent-loop/run` 和 `GET /api/weibo/agent-runs/:id`，只负责创建/查询 loop，不串完整业务 | API contract tests, MySQL unavailable tests | 1 | 低 | active |
