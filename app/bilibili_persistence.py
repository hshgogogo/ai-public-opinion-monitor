...
class InMemoryBilibiliRepository:
    def __init__(self):
        ...
        with self.db.connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    VALUES (%s,'bilibili',%s,%s,%s,%s,%s,'bilibili',%s,%s,%s,%s,%s)
                    ON DUPLICATE KEY UPDATE
                      id=LAST_INSERT_ID(id),
                      url=VALUES(url),
                    ...
                    (
                        ...
                        int(metrics.get("like_count") or metrics.get("view_count") or 0),
        ...
        with self.db.connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    VALUES (%s,%s,'bilibili',%s,%s,%s,%s,%s,%s,%s)
                    ON DUPLICATE KEY UPDATE
                      id=LAST_INSERT_ID(id),
...
def _validate_payload(payload):
    if payload.get("platform") != PLATFORM:
        return {
            "ok": False,
            "error_type": "bilibili_platform_not_allowed",
            "message": "Bilibili persistence only accepts normalized B站 payloads.",
        }
    ...
    if not isinstance(project_id, int) or project_id <= 0:
        return {
            ...
            "message": "Bilibili persistence requires a positive project_id.",
        }
    if not _payload_is_safe(payload):
        return {
            "ok": False,
            "error_type": "unsafe_bilibili_payload",
            "message": "Bilibili persistence rejected unsafe public payload values.",
        }
    return None
...
def _engagement(metrics):
    return int(metrics.get("like_count") or metrics.get("view_count") or 0)
