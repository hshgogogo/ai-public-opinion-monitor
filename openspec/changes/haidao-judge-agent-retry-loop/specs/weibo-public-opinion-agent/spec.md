...
- **WHEN** a Weibo comment analysis step is attached to an Agent Loop run and its output lacks valid comment or analysis evidence IDs
- **THEN** the Judge retry loop MUST reject the step output
- **AND** the Agent Loop MUST NOT present the analysis step as fully accepted until a passed Judge review exists or the step enters manual handling
...
- **WHEN** an event-building step claims a formal event without valid comment, analysis, or event evidence IDs
