...
- **WHEN** a Strategy Agent action suggestion lacks valid project evidence references such as target, post, comment, analysis, event, action, or memory IDs
- **THEN** the Judge retry loop MUST reject the suggestion
...
- **WHEN** a Strategy Agent action suggestion only cites `knowledge-card-*` references and has no real project evidence ID
- **THEN** the Judge retry loop MUST reject the suggestion as evidence insufficient
...
- **WHEN** an action suggestion states that a publicity action alone caused public-opinion change without sufficient evidence and confounder handling
- **THEN** the Judge retry loop MUST reject the output
