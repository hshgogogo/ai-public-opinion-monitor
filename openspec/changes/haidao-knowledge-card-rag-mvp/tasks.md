...
- [ ] 5.8 实现 C 级来源 action 弱启发文案和真实 evidence IDs 保护。

## 6. Q&A 知识引用

- [ ] 6.1 先写 Q&A citation 测试，断言回答引用 knowledge_card_id 且将其标记为知识参考。
- [ ] 6.2 `weibo-bot-message` 可检索并引用知识卡。
- [ ] 6.3 Q&A 回答区分事实、团队偏好、知识参考、推断和建议。
- [ ] 6.4 先写证据不足测试，断言知识卡不能伪造当前微博事实。
- [ ] 6.5 实现证据不足时仍回答真实微博证据不足。
- [ ] 6.6 先写 C 级来源 Q&A 测试，断言 C 级来源只能标记为弱启发。
- [ ] 6.7 实现 C 级来源 Q&A 弱启发表达。
